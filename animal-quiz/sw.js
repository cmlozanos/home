...
self.addEventListener('fetch', function (event) {
  ...
  if (event.request.mode === 'navigate') {
    event.respondWith(fetch(event.request).then(function (response) { if (!response.ok) throw new Error('Navigation unavailable'); return response; }).catch(function () { return caches.open(CACHE).then(function (cache) { return cache.match(event.request).then(function (cached) { return cached || cache.match('index.html'); }); }); }));
  } else {
