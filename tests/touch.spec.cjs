...
test('native multitouch cuts fruit and creates two planets',async({page,context,browserName})=>{
  ...
  let touches=[{id:11,x:position.x-position.r*1.8,y:position.y,radiusX:5,radiusY:5},{id:12,x:position.w*.85,y:position.h*.7,radiusX:5,radiusY:5}];
  ...
  touches[0].x=position.x+position.r*1.8;
  touches[1].x-=10;
