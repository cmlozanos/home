...
self.addEventListener('fetch',function(e){
  ...
  if(e.request.method!=='GET'||url.origin!==scope.origin||url.pathname.indexOf(scope.pathname)!==0)return;
  if(e.request.mode==='navigate'){
    e.respondWith(fetch(e.request).then(function(r){if(!r.ok)throw new Error('offline');return r;}).catch(function(){return caches.open(CACHE).then(function(c){return c.match('index.html');});}));return;
  }
